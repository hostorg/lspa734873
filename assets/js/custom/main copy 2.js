...
CURRENT_MODE = "existing";

$("#templates").show();
$("#customize-template-builder").hide();
$("#wrapper").hide();
...
$(document).ready(function() {
    ...
    // Select "Use Existing Template" by default
    const firstCard = $('.template-card').first();

    $('.template-card').removeClass('active');
    firstCard.addClass('active');

    firstCard.find('input[name="templateOption"]').prop('checked', true);
...
$(document).on('click', '.template-card', function() {
    ...
    const selectedOption = radio.val();

    if (selectedOption === 'customize') {

        CURRENT_MODE = "customize";
...
for (let i = 1; i < 99999; i++) {
    ...
    // Show template gallery (default state)
    $('#templates').show();
...
function ChoosePagesForHeaderFooter(selectedValue) {
    ...
    $("#displayMessageId").show().html("Please select one of the Headers from the available options given below");
